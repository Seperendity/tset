...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # self.dw = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1, groups=hidden_features)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, H, W, mask: Optional[torch.Tensor] = None):
        ...
        x_cnn = window_reverse(x_cnn, self.window_size[0], H, W)
        x_cnn = x_cnn.view(x_cnn.shape[0], -1, H, W)

        # conv branch
        x_cnn = self.dwconv3x3(x_cnn)
        # GAP[2,96,1,1]        GAP = F.adaptive_avg_pool2d(x_cnn, 1)
        channel_interaction = self.channel_interaction(GAP)
...
if __name__ == '__main__':
    ...
    swin = swin_tiny_patch4_window7_224(1000)
    output=swin(input)
    print(output.shape)